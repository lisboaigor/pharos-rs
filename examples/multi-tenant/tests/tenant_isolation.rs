...
async fn tenants_cannot_see_each_others_notes() -> TestResult {
    ...
    let mut acme_note = Note::create("welcome", "Welcome to Acme", "Acme onboarding.");
    acme_notes.save(&mut acme_note).await?;
    let mut globex_note = Note::create("welcome", "Welcome to Globex", "Globex handbook.");
    globex_notes.save(&mut globex_note).await?;

    // Each tenant reads only its own row.
    let acme_view = acme_notes.find_by_id(&"welcome".to_string()).await?;
    let globex_view = globex_notes.find_by_id(&"welcome".to_string()).await?;
    assert_eq!(
    ...
    acme_notes.delete(&"welcome".to_string()).await?;
    assert!(
        acme_notes
