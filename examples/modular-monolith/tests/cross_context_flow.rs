...
#[tokio::test]
async fn placing_an_order_issues_an_invoice_in_billing() -> Result<(), Box<dyn std::error::Error>> {
    let app = Monolith::new();

    // No invoice before the order exists.
    assert!(app.invoice_for("order-9").await.is_none());

    app.place_order("order-9", "Grace Hopper", 9_900).await?;

    // The billing context reacted to OrderPlaced and issued a matching invoice.
    let invoice = app
        .invoice_for("order-9")
        .await
        .ok_or("billing should have issued an invoice")?;
