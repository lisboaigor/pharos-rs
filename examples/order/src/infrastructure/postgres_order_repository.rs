...
impl Repository<Order> for PostgresOrderRepository {
    ...
    async fn save(&self, aggregate: &mut Order) -> Result<(), RepositoryError<Self::Error>> {
        ...
        let expected = aggregate.version();
        self.save_in_tx(&mut tx, aggregate).await?;

        if let Err(e) = tx.commit().await {
            // The transaction did not commit; leave the in-memory aggregate
            // exactly as it was before the call.
            aggregate.set_version(expected);
            return Err(RepositoryError::Storage(storage_error(e)));
        }
        Ok(())
    }
...
fn row_to_item(row: PgRow) -> Result<OrderItem, PostgresOrderRepositoryError> {
    ...
    Ok(OrderItem::rehydrate(
        ItemId::from_uuid(item_id),
        description,
        Quantity::new(quantity as u32).map_err(|e| {
            PostgresOrderRepositoryError::InvalidData(format!("invalid quantity: {e}"))
        })?,
        Money::from_cents(unit_price_cents as u64),
    ))
}
