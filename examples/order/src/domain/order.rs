...
mod tests {
    ...
    fn new_order() -> Order {
        Order::create(CustomerId::new()).unwrap()
    }
    ...
    #[test]
    fn total_uses_integer_arithmetic() {
        let mut p = new_order();
        p.add_item(
            "A".into(),
            Quantity::new(3).unwrap(),
            Money::brl(10.10).unwrap(),
        )
        .unwrap();
        // 3 * 1010 cents = 3030, with no floating-point error
        assert_eq!(p.total().unwrap().cents(), 3030);
    }
    ...
    #[test]
    fn does_not_add_item_after_confirmation() {
        let mut p = new_order();
        p.add_item(
            "A".into(),
            Quantity::new(1).unwrap(),
            Money::brl(5.0).unwrap(),
        )
        .unwrap();
        p.confirm().unwrap();
        let r = p.add_item(
    ...
    #[test]
    fn cancel_changes_status_and_emits_event() {
        let mut p = new_order();
        let _ = p.drain_events(); // discard OrderCreated
        p.cancel("customer changed their mind".into()).unwrap();
