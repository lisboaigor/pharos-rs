...
#[tokio::test]
async fn dispatch_wraps_command_in_its_trace_span() {
    ...
    let order_id = dispatch(&handlers, CreateOrder { customer_id })
        .await
        .expect("create order");
    ...
    let total = query_dispatch(&query, GetOrderTotal { order_id })
        .await
        .expect("query total");
