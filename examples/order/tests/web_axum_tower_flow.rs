...
#[tokio::test]
async fn full_order_lifecycle_over_http() -> Result<(), Box<dyn std::error::Error>> {
    let app = test_app();
    ...
    let order_id: Uuid = serde_json::from_str(&body_string(response).await?)?;
    ...
    let response = app
        ...
        .oneshot({
            let Ok(req) = Request::builder()
                ...
                panic!("invalid URI in test fixture");
            };
            req
        })
        .await?;
    assert_eq!(response.status(), StatusCode::OK);
    let total: Option<u64> = serde_json::from_str(&body_string(response).await?)?;
    assert_eq!(total, Some(78_000));
    Ok(())
}

#[tokio::test]
async fn confirming_unknown_order_maps_to_404() -> Result<(), Box<dyn std::error::Error>> {
    let app = test_app();
    let response = app
        .oneshot(post_json(
            "/orders/confirm",
...
#[tokio::test]
async fn invalid_quantity_maps_to_422() -> Result<(), Box<dyn std::error::Error>> {
    let app = test_app();
    ...
    // Quantity 0 fails input validation (garde) → 422 Unprocessable Entity, not a 500.
    let response = app
        .oneshot(post_json(
            "/orders/items",
            format!(
...
#[tokio::test]
async fn command_handler_as_a_tower_service() -> Result<(), Box<dyn std::error::Error + Send + Sync>>
{
