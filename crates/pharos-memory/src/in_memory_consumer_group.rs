...
mod tests {
    ...
    #[tokio::test]
    async fn coordinates_assignments() -> Result<(), Box<dyn std::error::Error>> {
        let coordinator = InMemoryConsumerGroupCoordinator::new();
        let topics = vec!["orders".to_string(), "payments".to_string()];

        let assignments = coordinator.join("workers", "consumer-1", &topics).await?;

        assert_eq!(assignments.len(), 2);
        assert_eq!(coordinator.assignments("workers").await?.len(), 2);

        coordinator.leave("workers", "consumer-1").await?;
        assert!(coordinator.assignments("workers").await?.is_empty());
        Ok(())
    }
}
