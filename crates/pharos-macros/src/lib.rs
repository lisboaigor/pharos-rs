...
/// calls [`garde::Validate::validate`] automatically. The handler receives the
/// command only after validation passes; failures become `H::Error` via
/// `From<garde::Report>` before the handler is ever called.
///
    ...
    let validate_input_fn =
        if matches!(kind, Dispatchable::Command) && (has_validate || has_garde_fields(ast)) {
            quote! {
                fn validate_input(
                    &self,
                ) -> ::std::result::Result<(), ::pharos_app::ValidationError> {
                    ::garde::Validate::validate(self)
                        .map_err(::pharos_app::ValidationError::from)
                }
            }
        } else {
            quote!()
