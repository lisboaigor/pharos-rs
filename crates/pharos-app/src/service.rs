...
///
/// The function first drains the aggregate's pending events, saves the current
/// aggregate state through the repository (advancing its optimistic-concurrency
/// version), and then publishes each drained event through the event bus.
...
mod tests {
    ...
    async fn save_and_publish_persists_aggregate_advances_version_and_dispatches() {
        ...
        save_and_publish(&repo, &bus, &mut aggregate).await.unwrap();
