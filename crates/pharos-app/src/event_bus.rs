...
impl EventBus {
    ...
    ///
    /// Dispatch is fail-fast: handlers run sequentially in registration order
    /// and the first error stops the run, so handlers registered after the
    /// failing one do not see the event. Callers that retry a failed publish
    /// re-deliver the event to every handler, including those that already
    /// succeeded — handlers must therefore be idempotent.
