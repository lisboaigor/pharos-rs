    ...
    pub async fn dispatch_pending(&self, limit: usize) -> DispatchResult {
        async move {
            ...
            for outbox_message in pending {
                ...
                // make is `attempts + 1`. If it cannot be recorded, skip the
                // publish: publishing without counting the attempt would let a
                // poisoned message retry forever and never reach the
                // dead-letter path.
                        ...
                        match self.config.retry.decide(attempt) {
                            ...
                            RetryDecision::DeadLetter => {
                                if let Err(mark_error) = self
                                    ...
                                    .await
                                {
                                    result.errors.push(mark_error.into());
                                }
                                metrics::counter!("pharos.outbox.dead_lettered").increment(1);
                            }
                            // Still retriable: leave the row `pending` and push
                            // its next attempt out by the computed backoff, so
                            // the policy's delay is honored across polls. A
                            // scheduling failure is recorded but non-fatal: the
                            // message is retried on a later poll regardless.
                            RetryDecision::RetryAfter(delay) => {
                                if let Err(error) =
                                    self.repo.schedule_retry(outbox_message.id, delay).await
                                {
                                    result.errors.push(error.into());
                                }
