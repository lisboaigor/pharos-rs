...
/// adapters (HTTP, tower, tests) depend on this type, not on the validation
/// library that produced it. Today it wraps a [`garde::Report`], but that is
/// an implementation detail — swap the validator and the port stays stable.
#[derive(Debug)]
pub struct ValidationError(garde::Report);

impl ValidationError {
    /// Returns the underlying garde report with per-field violations.
    pub fn report(&self) -> &garde::Report {
        &self.0
    }
}

impl From<garde::Report> for ValidationError {
...
pub trait Command: Send + Sync + 'static {
    ...
    /// fields. Adding [`#[derive(Validate)]`][garde::Validate] on the same struct
    /// is still required — it is what makes the generated code compile.
    ///
    /// ```ignore
