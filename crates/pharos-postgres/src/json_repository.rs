    ...
    <<A as Entity>::Id as FromStr>::Err: Display + Send + Sync + 'static,
{
    ...
    async fn save_in_tx(
        ...
        // Same contract as `save`: serialize with the new version applied,
        // revert it on failure so the caller retries from intact state.
        ...
        .map_err(|error| {
            aggregate.set_version(expected);
            match error {
                crate::PostgresTransactionError::ConcurrencyConflict { expected, actual } => {
                    RepositoryError::ConcurrencyConflict { expected, actual }
                }
                crate::PostgresTransactionError::Storage(e)
                | crate::PostgresTransactionError::Transaction(e) => {
                    RepositoryError::Storage(PostgresRepositoryError::Storage(e))
                }
                crate::PostgresTransactionError::Serialization(e) => {
                    RepositoryError::Storage(PostgresRepositoryError::Serialization(e))
                }
            }
        })
