...
| Schemaless cache values, Redis messages with dynamic payloads | roll your own `MessageCodec` |
...
| Serializer                | Payload bound (`impl`)                   | Feature |
| ------------------------- | ---------------------------------------- | ------- |
| `JsonEventSerializer`     | `Serialize + DeserializeOwned + 'static` | default |
| `ProtobufEventSerializer` | `prost::Message + Default + 'static`     | `proto` |
