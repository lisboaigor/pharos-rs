...
| Transactions                | `TransactionalRepository` + `save_and_enqueue_in` give any repository (JSONB or explicit relational) the atomic save+outbox guarantee | Transaction-per-command policy is composed by the application (a Tower layer), not built in |
